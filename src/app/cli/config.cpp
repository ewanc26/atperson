...
std::filesystem::path jetstream_kinds_path() {
    const std::string configured = env_or("ATPERSON_JETSTREAM_KINDS_FILE");
    return configured.empty() ? std::filesystem::path{} :
                                std::filesystem::path(configured);
}
...
std::filesystem::path authorization_envelopes_path() {
    return env_or("ATPERSON_AUTHORIZATION_ENVELOPES",
                  (data_dir() / "envelopes").string());
}

std::filesystem::path offline_spool_path() {
    return env_or("ATPERSON_OFFLINE_SPOOL",
                  (data_dir() / "offline-spool").string());
}
