...
/* Generic NODES: each node carries its embedding and the per-dimension
 * embedding-importance vector at the ARCH-persisted width. */
static bool atp_decode_nodes_v6(atp_reader *reader, atp_graph *graph) {
    uint64_t count = 0u;
    const uint64_t min_entry = 4u + 8u + 4u +
                               (uint64_t)graph->neural_architecture.embedding_dim * 8u + 1u;
    ...
    for (size_t i = 0u; i < (size_t)count; ++i) {
        ...
        for (size_t d = 0u; d < graph->neural_architecture.embedding_dim; ++d) {
            if (!atp_reader_f32(reader, &node->embedding[d]) ||
                !atp_reader_f32(reader, &node->embedding_importance[d])) {
                return false;
            }
        }
    }
...
atp_graph *atp_load_v6(const unsigned char *data, size_t size, atp_status *status) {
    ...
    while (reader.size - reader.position > 8u) {
        ...
        case ATP_SECTION_ARCH: {
            ...
            if (graph || !seen[ATP_SECTION_HEADER]) {
                break;
            }
        ...
        case ATP_SECTION_NODES:
            ok = graph && atp_decode_nodes_v6(&reader, graph);
            break;
