 ...
 * the in-memory dedup index; recover.c owns crash recovery; migrate.c owns
 * legacy-format migration (v1/v2 -> current).
 *
...
#define ATP_LEDGER_FILE_MAGIC_7 '3'
...
#define ATP_LEDGER_OFF_MAGIC_7 '3'
...
/*
 * Entry records carry three length-prefixed conversational-context URIs
 * (reply root, reply parent, quote), each framed as a u32 length plus at most
 * ATPERSON_CONTEXT_URI_BYTES-1 bytes of text. Reserve the full cap so the
 * writer never depends on how much of a URI the caller supplied.
 */
#define ATP_LEDGER_CONTEXT_MAX (3u * (4u + ATPERSON_CONTEXT_URI_BYTES))
#define ATP_LEDGER_BODY_MAX \
...
/* recover.c: crash recovery of the current-format log. */
atp_status atp_ledger_recover(atp_ledger *ledger);

/* migrate.c: legacy-format migration to the current version. */
