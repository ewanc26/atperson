...
When the synthetic vectors above pass, validate against real known-good data from the reference implementations. In atperson the encoder is Wolfram; use this section to double-check Wolfram output against upstream fixtures, not to build a local encoder.

- **indigo (Bluesky, Go)** — `indigo/atproto/repo/testdata/` and `indigo/atproto/repo/mst/testdata/` hold real-world repo CARs, MST node fixtures, and interop vectors.
- **`@atproto/repo` (TypeScript)** — `atproto/packages/repo/tests/*.test.ts` exercises real commit signing, MST mutation sequences, and CAR round-trips with shared fixture builders.
- **`atproto-repo` (Rust, ngerakines.me)** — `atproto-identity-rs/crates/atproto-repo/tests/` carries the cross-language interop fixtures, including `prev` divergence cases.
- **Wolfram** — the pinned revision parsed by this project; cross-check its parse output against at least one upstream fixture above.

If Wolfram output disagrees with an upstream fixture at the byte level, treat it as a Wolfram bug to report upstream, not something atperson should paper over.
