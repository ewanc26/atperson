...
How a consumer takes an NSID and returns a lexicon document. Understand the chain when debugging why a resolution fails; atperson performs the fetch leg via Wolfram (`wf_agent_get_record` / `wf_sync_get_record`) and never publishes.
...
```
wf_agent_get_record(agent, "com.atproto.lexicon.schema", "<nsid>", &resp)
# or raw XRPC: wf_xrpc_query(client, "com.atproto.repo.getRecord", params, resp)
```
...
The `value` field is a valid lexicon document — same shape as if you'd loaded it from a file. Feed it into the C++23 runtime's `wf_lexicon_registry` (`wf_lexicon_registry_load`), see `skills/atproto-lexicon` for catalog idioms.
...
2. Does the DID resolve? Use `wf_did_resolve` (see `atproto-identity-resolution`).
...
5. Does a reference resolver return the record while your local chain failed? If yes, the difference is in step 1 or 2 (DNS resolver or identity resolver).

## See also

- `record-shape.md` — what the fetched record looks like.
- `authority-and-ownership.md` — why step 2 gates everything.
- `skills/atproto-identity-resolution/SKILL.md` — step 3 in full.
- `skills/atproto-lexicon/shared/nsid.md` — NSID grammar and reserved prefixes.
