...
Use the authoritative matrix in `skills/atproto-lexicon/shared/backward-compat.md`. It covers every change kind (add/remove/mutate field, tighten/loosen constraints, open/closed unions, method parameter changes, etc.).
...
Run the compatibility check by hand against the matrix in
`skills/atproto-lexicon/shared/backward-compat.md` — there is no equivalent of
a `check_compatibility` tool in the pinned Wolfram library; validate each
document in isolation via `wf_validate_record` and diff the shapes yourself:

```
old = getRecord(at://<did>/com.atproto.lexicon.schema/<nsid>).value
new = <the doc you're about to publish>
offline: validate both through a wf_lexicon_registry, then
         diff add/remove/change of defs + required fields
```

If the diff flags breaks:
...
3. Within minutes, re-resolution (`getRecord` of the schema record) returns the new document to consumers who refetch.
...
- `skills/atproto-lexicon/shared/backward-compat.md` — full matrix of breaking vs non-breaking changes.
