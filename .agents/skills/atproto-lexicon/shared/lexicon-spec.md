...
`type: "unknown"` is an escape hatch. It matches any DAG-CBOR/JSON value and is **not** recursively validated by default. Implementations that look inside `unknown` values for a `$type` and perform nested validation do so as an extension, not by spec mandate.
...
Tracked here so Claude doesn't assume the spec resolves every edge case:
...
4. Strict-vs-lenient unknown-field handling is not mandated; atperson pins it to Wolfram's `wf_validate_record` behavior.
