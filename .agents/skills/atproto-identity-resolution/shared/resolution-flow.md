...
## 8. Resolution in atperson (Wolfram)

atperson resolves through Wolfram, not MCP servers:

- `wf_handle_resolve(client, handle)` — DNS TXT first, then HTTPS well-known.
- `wf_did_resolve(client, did)` — DID document fetch + parse; `wf_did_resolve_raw`
  for the untrimmed JSON when `alsoKnownAs` must be audited.
- `wf_handle_parse_dns_txt` — the "exactly one `did=` record" DNS rule.
- `wf_identity_verify_handle(client, did, handle)` — `com.atproto.identity.verifyHandle`
  XRPC for the server-verified path.

The reference implementations named in §1–§7 remain the conformance target for
behavior (DNS-vs-HTTPS ordering, conflict handling, DID-method gating); tests
may diff Wolfram results against those documented behaviors, but atperson ships
no separate resolver stack.
