...
- **Validate the client metadata JSON** on every CI/startup path with `wf_oauth_client_metadata_parse` (Wolfram), plus a hand review against `shared/client-metadata.md`.
...
- [ ] Client metadata validated with `wf_oauth_client_metadata_parse` (WF_OK) in CI.
