...
- **Client-metadata document** fixtures: hand-authored, conformant to the invariants in `shared/client-metadata.md` and passable through `wf_oauth_client_metadata_parse`.
...
**Expected validation result:** PASS. Run through `wf_oauth_client_metadata_parse` (WF_OK) in a build-core test and against the invariants in `shared/client-metadata.md`.
