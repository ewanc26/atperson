...
The Smoke Signal reference BFF uses lazy refresh in middleware — check on every inbound request, refresh before dispatching. In atperson, session refresh is Wolfram's (`wf_oauth_session_refresh` / `wf_session_refresh`) if an OAuth session is ever used; today atperson logs in with legacy app passwords.
