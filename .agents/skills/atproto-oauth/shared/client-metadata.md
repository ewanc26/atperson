...
Before serving the document from your own service (or when auditing one fetched from the network), validate it with Wolfram's `wf_oauth_client_metadata_parse` (WF_OK), then review the parsed fields against the invariants below:
