...
When writing a new implementation, producing a signed record via the CLI and verifying it via your implementation (and vice versa) is the primary interop test. The reference crate's behavior (and its known gaps) in `shared/spec.md` §curve-support and `shared/signature-normalization.md` are the interop target.
...
- Reference crate source: <https://tangled.org/ngerakines.me/atproto-crates/tree/main/crates/atproto-attestation>.
